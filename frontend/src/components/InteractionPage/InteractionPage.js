...
const InteractionPage = ({ setCompleted }) => {
  const { id } = useParams();
  ...
  let interactionObj = data.places.reduce((interactionObj, currentPlace) => {
    let foundInteraction = currentPlace.interactions.find((interaction) => interaction.id === interactionId);
  ...
  useEffect(() => {
    ...
    shuffledAnswersRef.current = shuffleArray(interactionObj.answers);
  ...
  useEffect(() => {
    ...
    if (correctAnswersIds.every((correctId) => selectedAnswers.includes(correctId))) {
      setCompleted((completed) => (completed.includes(interactionId) ? completed : addStar(interactionId, completed)));
      setIsCompleted(true);
    }
  }, [selectedAnswers, interactionId, interactionObj.answers, interactionObj.hints, setCompleted]);
